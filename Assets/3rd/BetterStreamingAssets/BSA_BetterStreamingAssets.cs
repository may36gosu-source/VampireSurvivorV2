...
public static class BetterStreamingAssets
{
    ...
    internal static class LooseFilesImpl
    {
        ...
        public static string[] GetFiles(string path, string searchPattern, SearchOption searchOption)
        {
            ...
            path = Better.StreamingAssets.PathUtil.NormalizeRelativePath(path, forceTrailingSlash : true);
        ...
        {
            path = Better.StreamingAssets.PathUtil.NormalizeRelativePath(path);
        ...
        public static bool DirectoryExists(string path)
        {
            var normalized = Better.StreamingAssets.PathUtil.NormalizeRelativePath(path);
    ...
    internal static class ApkImpl
    {
        ...
        public static bool DirectoryExists(string path)
        {
            var normalized = Better.StreamingAssets.PathUtil.NormalizeRelativePath(path, forceTrailingSlash : true);
        ...
        public static string[] GetFiles(string path, string searchPattern, SearchOption searchOption)
        {
            ...
            var actualDirPath = Better.StreamingAssets.PathUtil.NormalizeRelativePath(path, forceTrailingSlash : true);
            //Debug.Log("GetFiles actualDirPath:"+ actualDirPath);
            ...
            {
                var regex = Better.StreamingAssets.PathUtil.WildcardToRegex(searchPattern);
            ...
            for ( int i = index; i < s_paths.Length; ++i )
            {
                ...
                if ( filter == null || filter(fileName) )
                {
                    ...
                    if ( fixedPath == null )
                    {
                        fixedPath = Better.StreamingAssets.PathUtil.FixTrailingDirectorySeparators(path);
                        if ( fixedPath == "/" )
                            fixedPath = string.Empty;
                    }

                    var result = Better.StreamingAssets.PathUtil.CombineSlash(fixedPath, normalizedPart);
                    results.Add(result);
                }
            }
